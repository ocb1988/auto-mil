...
try:
    ...
    class SAGEConv(nn.Module):
        ...
        def forward(self, x, edge_index):
            ...
            counts.index_add_(0, row, torch.ones(row.numel(), 1, device=x.device, dtype=x.dtype))
        ...
        def forward(self, x, batch=None):
            if batch is not None:
                ...
                for batch_id in batch.unique(sorted=True):
                    mask = batch == batch_id
                    weights = torch.softmax(self.gate_nn(x[mask]), dim=0)
                    outputs.append((weights * x[mask]).sum(dim=0, keepdim=True))
                return torch.cat(outputs, dim=0)
            weights = torch.softmax(self.gate_nn(x), dim=0)
            return (weights * x).sum(dim=0, keepdim=True)

    AttentionalAggregation = GlobalAttention
