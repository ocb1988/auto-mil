...
def cal_scores(probs, labels, num_classes):       # probs:[batch_size, num_classes]   labels:[batch_size, ]
    ...
    try:
        if num_classes > 2:
            ...
            macro_auc = roc_auc_score(y_true=labels.numpy(), y_score=probs.numpy(), average='macro', multi_class='ovr', labels=metric_labels)
            micro_auc = roc_auc_score(y_true=labels.numpy(), y_score=probs.numpy(), average='micro', multi_class='ovr', labels=metric_labels)
            weighted_auc = roc_auc_score(y_true=labels.numpy(), y_score=probs.numpy(), average='weighted', multi_class='ovr', labels=metric_labels)
        else:
            macro_auc = roc_auc_score(y_true=labels.numpy(), y_score=probs[:,1].numpy())
            weighted_auc = micro_auc = macro_auc
    except ValueError:
        macro_auc = micro_auc = weighted_auc = float('nan')
