...
def propose_nodes(
    ...
    *,
    ...
    if baseline_plan:
        candidates.extend(_family_candidates(cfg, tree, baseline_plan, max_proposals))
...
def _render_markdown(summary: ProposalSummary) -> str:
    lines = [
        ...
        "",
        "| Added | Node | Type | Model | Prior | Overrides | Rationale |",
        "|---|---|---|---|---:|---|---|",
    ]
    for item in summary.proposals:
        rationale = item.rationale.replace("|", "/")
        lines.append(
            f"| {str(item.added).lower()} | `{item.node_id}` | {item.proposal_type} | "
            f"{item.model_name} | {item.prior:.2f} | `{item.config_overrides}` | {rationale} |"
        )
    if summary.warnings:
